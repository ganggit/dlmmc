...
% pX = pX';
[N, numdims] = size(train_X);

if length(network)==3

XX = [train_X ones(N,1)];
nid =1;
w1 = [network{nid}.W; network{nid}.bias_upW];
nid =2;
w2 = [network{nid}.W; network{nid}.bias_upW];
nid =3;
w3 = [network{nid}.W; network{nid}.bias_upW];
...
% while (innerConvergence == 0)
    ...
    allW = dvWh(:);
    elseif length(network) ==3
    
        Ix3 = dX.*w3probs.*(1-w3probs); 
        dw3 =  [w2probs]'*Ix3;
        
        Ix2 = (dX*w3').*w2probs.*(1-w2probs); 
        Ix2 = Ix2(:,1:end-1);
        dw2 =  w1probs'*Ix2;

        Ix1 = (Ix2*w2').*w1probs.*(1-w1probs); 
        Ix1 = Ix1(:,1:end-1);
        dw1 =  [train_X, ones(nPoints, 1)]'*Ix1;
