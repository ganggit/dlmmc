...
addpath('/gpfs/scratch/gangchen/GangChen_CVPR/RBMLIB/spectral-0.2');
...
dataid = 'coil_100_tSNE';
% fpath = '/panfs/panfs.ccr.buffalo.edu/scratch/GangChen_CVPR/dataset/';
fpath = '/gpfs/scratch/gangchen/GangChen_CVPR/dmmc/drtoolbox/';
fname = 'mappedX2.mat';
...
% train_X = projection(network, X', L-1);
% train_X = train_X';
maxiter =50;
% -----------------------------------------------------------
...
dist = cdistM(A,train_X,idxa_test,idxb_test); 
...
for c=1:length(un)
    ...
    for nameCounter=1:length(names)
        ...
        dist = cdistM(M,train_X,idxa_test,idxb_test); 
        [tpr, fpr] = icg_roc(matches,-dist);
        [ignore, eerIdx] = min(abs(tpr - (1-fpr)));
        %eer
        eer =tpr(eerIdx);
...
save([dataid '_' strnum '_DBN.mat'], 'Accuracy', 'rocval');
