function [newW_vec, vWh, objFunVal,network] = extendedPEGASOS_dbn_2(train_X,network, nClusters,...
...
while (innerConvergence == 0)
    ...
    vWh = vWh- vWhinc;
    % ----- previous one ------------
    %dinc = -0.1/s * dvWh; %momentum * vWh -0.01 * dvWh;
    %vWh = vWh +  dinc; %vWhinc;
    if mod(flag,2)==0  
    ...
    %///////////////////////////////////////////////////////////////////////////////////////////
    if norm(newW_vec - W_vec) ./max( norm(newW_vec), norm(W_vec) ) <= innerTol | s > 500%5000%round(100./lambda)%500     
        innerConvergence = 1;
