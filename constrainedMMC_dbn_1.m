function [clusterLabel, W, CCCP_objFunVal, network, elapsedTime]=constrainedMMC_dbn_2(train_X,network, nClusters, lambda, cOne, T, option)
...
% get the size
[nPoints, dim] = size(X);
X = X - repmat(mean(X,1), nPoints, 1);
% transfer it into the mmc format
X = double(X');
...
while( outterConvergence == 0 )
    ...
    [newW_vec, vWh, newObjFunVal] = extendedPEGASOS_dbn_2(train_X,network, nClusters,...
