function [clusterLabel, W, CCCP_objFunVal, elapsedTime]=constrainedMMC_hidden(train_X,network, nClusters, lambda, cOne, T, option)



max_iter =200;

addpath('/Users/gangchen/Downloads/RBM/hucrf_code/minFunc');
...
while( outterConvergence == 0 )
    ...
    allW = [];
    for nid =1: length(network)-1 % do not consider the top layer
        vWh  = [network{nid}.W(:); network{nid}.bias_upW(:)];
        allW = [allW;vWh(:)];
    end
...
%     [newW_vec, vWh, newObjFunVal] = extendedPEGASOS_hidden2(train_X,network, nClusters,...
...
%                               
    [x, newObjFunVal] = minFunc(@mmc_grad_func, [W_vec(:); allW(:)], options,...
        train_X,network, nClusters,Y_t, Z_minus_t, Z_plus_t, ...
        I_NonMC_vec,I_C_vec, J_C_vec,I_M_vec, J_M_vec, initW_vec, lambda, innerTol, preIteration, t, cOne);
    
    % reshape x back to the data 
    indx =1;
    % newW_vec = reshape(x(indx:indx+numel(W)-1), [dim nClusters]); 
    newW_vec = x(indx:indx+numel(W)-1);
    indx = indx + numel(W);
    for nid =1: length(network)-1 % donot consider the top layer
    network{nid}.W = reshape(x(indx:indx+ numel(network{nid}.W)-1), size(network{nid}.W)); 
    indx = indx + numel(network{nid}.W);
    network{nid}.bias_upW = reshape(x(indx:indx+ numel(network{nid}.bias_upW)-1), size(network{nid}.bias_upW)); 
    indx = indx+ numel(network{nid}.bias_upW);
    end
    t = t + 1;
